...
import DashboardCardSkeleton from "../../(user)/user-dashboard/(post-management)/my-posts/_components/CardSkeleton";
import { useGetAdminDashboardAnalyticMatrix } from "@/src/hooks/admin.analytics.hook";
...
const AdminDashboard = () => {
  const {user} = useUser()
  const { data,isLoading, isPending } = useGetAdminDashboardAnalyticMatrix();

  // Assuming data has an array of actions
  const adminAnalytics = data?.data || [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="!bg-primaryColor text-white p-6 rounded-xl shadow-lg flex flex-col justify-between">
          <div>
            ...
            <h3 className="text-3xl font-bold mt-2">{user?.username}</h3>
